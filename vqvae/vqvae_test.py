...
def test_vqvae(vqvae, model_path, test_loader, indices=None):
    ...
    metrics = score(np.zeros_like(test_dataset[0][0]), np.zeros_like(test_dataset[0][0]), True)
    selected_images = []
    ...
    display_results(selected_images, save_path="results.png")
...
if __name__ == "__main__":
    ...
    test_dataset = SingleImageDataset("/media/viplab/DATADRIVE1/skin_lesion/ISIC2018/Test_GroundTruth/", task="image", transform=transform)
    ...
    vqvae = VQVAE(in_channels=1, vocab_size=8192, z_channels=64, ch=128, test_mode=True)
    test_vqvae(vqvae, "./checkpoints/8192_noconv.pth", test_loader, indices=test_indices)
